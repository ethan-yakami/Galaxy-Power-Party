import { installAuthRuntime } from './auth-runtime.js';
...
import { installUrlUtils } from './url-utils-runtime.js';
...
async function bootstrapLauncherApp(windowRef) {
  ...
  const urls = installUrlUtils(windowRef);
  const authApi = installAuthRuntime(windowRef, urls);
  frontendDiagnostics.runtimeSources.critical = 'native-ready';
  ...
  createLauncherApp({
    authApi,
    documentRef: windowRef.document,
    shellApi: windowRef.GPPShell,
    urls,
    windowRef,
  });
  frontendDiagnostics.runtimeSources.launcher = 'native-ready';
