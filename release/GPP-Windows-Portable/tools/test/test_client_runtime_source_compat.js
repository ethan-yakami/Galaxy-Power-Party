...
async function run() {
  ...
  try {
    ...
    assert(launcherEntry.text.includes('createLauncherApp'), 'launcher-entry.js should bootstrap the launcher from native modules');
    assert(!launcherEntry.text.includes('runtime-source-loader'), 'launcher-entry.js should not depend on runtime-source-loader anymore');
