...
const CONTENT_DIR = path.join(ROOT, 'src', 'content', 'entities');

// 1. Update gradients in styles.css to be white on left
let styles = fs.readFileSync(path.join(CLIENT_DIR, 'styles.css'), 'utf8');
...
fs.writeFileSync(path.join(CLIENT_DIR, 'styles.css'), styles);

// 2. Clear custom_characters (yaoguang v2, xilian v2)
fs.writeFileSync(path.join(CONTENT_DIR, 'custom_characters.json'), JSON.stringify({ variants: [] }, null, 2));

// 3. Remove "路" text inside render.js
let renderJs = fs.readFileSync(path.join(CLIENT_DIR, 'js', 'render.js'), 'utf8');
renderJs = renderJs.replace(/ 路 /g, '    ');
fs.writeFileSync(path.join(CLIENT_DIR, 'js', 'render.js'), renderJs);

// 4. Update weatherBroadcast duration in ui.js
let uiJs = fs.readFileSync(path.join(CLIENT_DIR, 'js', 'ui.js'), 'utf8');
...
fs.writeFileSync(path.join(CLIENT_DIR, 'js', 'ui.js'), uiJs);
