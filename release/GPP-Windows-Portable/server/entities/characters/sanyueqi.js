  ...
  hooks: {
    onMainAttackConfirm(game, attacker, selectedDice, room) {
      ...
      if (pairs > 0) {
        ...
        pushEffectEvent(game, { type: 'instant_damage', sourcePlayerId: attacker.id, targetPlayerId: target.id, amount: dmg, hpBefore: before, hpAfter: game.hp[target.id] });
    ...
    onDefenseConfirm(game, defender, selectedDice, room) {
      ...
      if (pairs > 0) {
        ...
        const before = game.hp[target.id];
        game.hp[target.id] -= dmg;
        pushEffectEvent(game, { type: 'instant_damage', sourcePlayerId: defender.id, targetPlayerId: target.id, amount: dmg, hpBefore: before, hpAfter: game.hp[target.id] });
