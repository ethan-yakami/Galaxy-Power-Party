...
describe('launcher auth/public rooms linkage', () => {
  ...
  it('renders joinableReason and blocks joining non-joinable known rooms', async () => {
    ...
    const blockedOption = Array.from(select.options).find((option) => option.value === '2222');
    expect(blockedOption).toBeTruthy();
    expect(blockedOption?.disabled).toBe(true);
    expect(blockedOption?.textContent).toContain('对局进行中');

    const roomCodeInput = /** @type {HTMLInputElement} */ (document.getElementById('roomCodeInput'));
    roomCodeInput.value = '2222';
    ...
    expect(messageText).toContain('对局进行中');
  });
});
