...
const CUSTOM_CHARACTER_STORE_VERSION = 'CustomCharacterStoreV2';
...
function normalizeStoredVariant(item) {
  if (!isPlainObject(item)) return null;
  const id = typeof item.id === 'string' ? item.id.trim() : '';
  const baseCharacterId = typeof item.baseCharacterId === 'string' ? item.baseCharacterId.trim() : '';
  if (!id || !baseCharacterId) return null;
  return {
    id,
    baseCharacterId,
    name: typeof item.name === 'string' ? item.name.trim() : '',
    enabled: item.enabled !== false,
    overrides: isPlainObject(item.overrides) ? cloneJson(item.overrides) : {},
    updatedAt: Number.isFinite(item.updatedAt) ? item.updatedAt : Date.now(),
    createdAt: Number.isFinite(item.createdAt) ? item.createdAt : Date.now(),
  };
}

function normalizeCustomCharacterStore(parsed) {
  if (!isPlainObject(parsed)) {
    return {
      version: CUSTOM_CHARACTER_STORE_VERSION,
      characters: {},
    };
  }

  if (parsed.version === CUSTOM_CHARACTER_STORE_VERSION && isPlainObject(parsed.characters)) {
    const characters = {};
    for (const [id, raw] of Object.entries(parsed.characters)) {
      const normalized = normalizeStoredVariant(Object.assign({}, raw, { id }));
  ...
  for (const rawVariant of variants) {
    const normalized = normalizeStoredVariant(rawVariant);
    if (!normalized) continue;
    characters[normalized.id] = normalized;
  }
  return {
...
function readCustomCharacterStore() {
  if (!fs.existsSync(CUSTOM_CHAR_PATH)) {
    return {
      version: CUSTOM_CHARACTER_STORE_VERSION,
      characters: {},
    };
  ...
  try {
    ...
    return normalizeCustomCharacterStore(parsed);
  } catch (err) {
...
function writeCustomCharacterStore(store) {
  const normalized = normalizeCustomCharacterStore(store);
  fs.mkdirSync(path.dirname(CUSTOM_CHAR_PATH), { recursive: true });
  fs.writeFileSync(CUSTOM_CHAR_PATH, JSON.stringify(normalized, null, 2), 'utf8');
}

function loadCustomCharacterVariants() {
  return Object.values(readCustomCharacterStore().characters);
}
...
function getCharacterSummary() {
  return Object.values(CharacterRegistry).map((c) => ({
    ...
    allowsNoAurora: allowsNoAurora(c),
  }));
}
...
function listCustomVariants() {
  return Object.values(readCustomCharacterStore().characters)
    .sort((a, b) => String(a.name || a.id).localeCompare(String(b.name || b.id), 'zh-Hans-CN'));
}

function getCustomVariant(id) {
  const safeId = typeof id === 'string' ? id.trim() : '';
  if (!safeId) return null;
  return readCustomCharacterStore().characters[safeId] || null;
}

function upsertCustomVariant(variant) {
  const normalized = normalizeStoredVariant(variant);
  if (!normalized) {
    throw new Error('Invalid custom variant payload.');
  }
  const store = readCustomCharacterStore();
  const existing = store.characters[normalized.id];
  if (existing && Number.isFinite(existing.createdAt)) {
    normalized.createdAt = existing.createdAt;
  }
  normalized.updatedAt = Date.now();
  store.characters[normalized.id] = normalized;
  writeCustomCharacterStore(store);
  reloadRegistry();
  return cloneJson(normalized);
}

function removeCustomVariant(id) {
  const safeId = typeof id === 'string' ? id.trim() : '';
  if (!safeId) return false;
  const store = readCustomCharacterStore();
  if (!store.characters[safeId]) return false;
  delete store.characters[safeId];
  writeCustomCharacterStore(store);
  reloadRegistry();
  return true;
}

function toggleCustomVariant(id, enabled) {
  const safeId = typeof id === 'string' ? id.trim() : '';
  if (!safeId) return null;
  const store = readCustomCharacterStore();
  const current = store.characters[safeId];
...
function saveCustomVariant(variant) {
  return upsertCustomVariant(variant);
}
